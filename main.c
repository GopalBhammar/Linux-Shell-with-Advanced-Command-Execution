    ...
    while(1){
        ...
        // tokenise using white space
            ...
            while (tok != NULL) {
                ...
                else if(tok[strlen(tok)-1]=='|')last=1;
                ...
                while (subtok != NULL) {
                    ...
                    arg_count++;
                    args1[command_count][arg_count]= NULL;
                    if(first_arg_count==0)first_arg_count=arg_count;
        ...
        args1[command_count][arg_count]=NULL;
        if(command_count)command_val=5; // pipe
        
        else{
            if(first_arg_count<=1){
                command_val = command_list(args1[0][0],"dummy");} //single commands
            else{
            ...
            case 7://vi with argument
                    ...
                    while (1) {
                        ...
                        if (ch == KEY_UP && y > 0) {
                            ...
                            if(isalnum(getyx(stdscr, y, x)))num_chars--;
                            if (x > 0) {
                                move(y, x - 1);
                                delch();
                                x--;
            ...
            case 8://threads
                ...
                if (strcmp(operation, "addvec") == 0) {
                    ...
                    for (int i = 0; i < num_threads; i++) {
                        task.thread_id = i;
                        pthread_create(&threads[i], NULL, Vector_add, &task);
                    }
                    ...
                    for (int i = 0; i < num_threads; i++) {
                        task.thread_id = i;
                        pthread_create(&threads[i], NULL, Vector_sub, &task);
                    }
                    ...
                    for (int i = 0; i < num_threads; i++) {
                        task.thread_id = i;
                        pthread_create(&threads[i], NULL, Vector_prod, &task);
                    }
